...
SemanticMemo uses embedding search to find candidate cache entries, but the equivalence
...
`src/semanticmemo/classifier/model.py`.

## The bundled classifier (`equivalence-net-v1`)

SemanticMemo ships a pretrained checkpoint inside the package at
`semanticmemo/_models/equivalence-net-v1.pt`. Load it with `ClassifierConfig.bundled()`.

`equivalence-net-v1` is a **generic cold-start model**. Its training data was built locally:
...
`semanticmemo/_models/equivalence-net-v1.report.json`.
...
- Being generic, it trades per-domain peak accuracy for broad coverage. Per-domain
  accuracy improves once you collect feedback and run `semanticmemo retrain` — see
  `docs/ml/training-your-own.md`.

At runtime, `SemanticMemo(..., classifier=ClassifierConfig.bundled())` (or any
